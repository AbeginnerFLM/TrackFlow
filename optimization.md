...
1. **[高优先] WebSocket 发送安全收敛（连接生命周期保护）**
   - 现状：异步线程中捕获 `ws*` 后再 `loop->defer(send)`，连接若已关闭，存在潜在悬空指针风险。
   - 建议：引入连接代号（generation/token）或会话级发送队列，在主循环确认连接仍存活后再发送。
   - 收益：提高高并发断连场景下的稳定性，避免偶发崩溃。

2. **[中优先] Pipeline 配置哈希与惰性重建**
   - 现状：`get_or_create` 仅按 `session_id` 复用，配置更新时需要显式 reset。
   - 建议：为配置计算哈希，若同一 `session_id` 配置变化则按策略热更新（重建 pipeline 或局部重配）。
   - 收益：减少“配置漂移”带来的隐性行为不一致，降低运维复杂度。

3. **[中优先] YOLO 后处理向量化**
   - 现状：类别最大分数搜索为纯标量循环。
   - 建议：对 `num_classes` 分支引入 SIMD（AVX2/NEON）或 OpenCV `minMaxIdx` 批量策略。
   - 收益：多类别模型下可降低后处理 CPU 占用（典型 10%~25%）。

4. **[中优先] 响应序列化切换到二进制协议**
   - 现状：检测结果始终走 JSON 文本，帧频高时序列化开销明显。
   - 建议：引入 MessagePack/FlatBuffers（保留 JSON 兼容模式）。
   - 收益：带宽与序列化时间双降，远程链路（VPS/FRP）收益更明显。

5. **[低优先] 会话清理策略参数化**
   - 现状：会话过期窗口默认 300s，清理频率固定步长。
   - 建议：将 timeout 与清理步长暴露到配置，按部署规模动态调整。
   - 收益：长时间运行时内存曲线更平滑，利于不同负载场景调优。
